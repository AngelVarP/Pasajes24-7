...
<head>
    ...
    {{-- Favicon (asegúrate de tenerlo en public/images/p-favicon.png o ajusta la ruta) --}}
    <link rel="icon" href="{{ asset('images/p-favicon.png') }}" type="image/png" sizes="32x32">
    <link rel="icon" href="{{ asset('images/p-favicon.png') }}" type="image/png" sizes="16x16">
    <link rel="apple-touch-icon" href="{{ asset('images/p-favicon.png') }}">
    ...
    <style>
        ...
        .clip-path-fancy { clip-path: polygon(0 0, 100% 0, 100% 85%, 0% 100%); }
        @media (min-width: 768px) { .clip-path-fancy { clip-path: polygon(0 0, 100% 0, 100% 75%, 50% 100%, 0 75%); } }
    </style>
    {{-- @stack('styles') --}} {{-- No necesitamos stack si no usamos layout --}}
</head>
<body class="bg-gray-100 text-gray-800"> {{-- Quitamos flex flex-col min-h-screen --}}
    ...
    {{-- Contenido principal de Welcome (NO usa $slot) --}}
    <main> {{-- Quitamos flex-grow --}}
        {{-- Hero Section con forma y superposición --}}
        <section class="relative bg-gradient-to-br from-gray-800 via-gray-900 to-black text-white pb-24 md:pb-32 clip-path-fancy">
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-24 md:pt-32 text-center"> {{-- Ajustado pt para compensar navbar fijo --}}
                ...
                <p class="text-xl md:text-2xl font-bold text-amber-300 mb-6 drop-shadow">Tu aventura en Bus comienza Aquí.</p>
        ...
        <section class="relative z-20 -mt-20 md:-mt-28 pb-16">
            <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                <div id="search-form-container" class="max-w-3xl mx-auto bg-white p-6 sm:p-8 rounded-2xl shadow-xl border border-gray-100">
                    <form action="{{ route('viajes.buscar') }}" method="POST" class="space-y-5" id="search-form">
                         @csrf
                        {{-- Contenido del formulario (origen, destino, fecha, botón, imagen) --}}
    ...
    {{-- Footer (Propio de Welcome) --}}
    <footer class="bg-gray-900 text-gray-400 py-10 mt-16">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div class="mb-6">
                 {{-- Usa tu logo --}}
                 <img src="{{ asset('images/logo-pasajes24-7-2.png') }}" alt="Logo Pasajes24/7" class="h-8 w-auto mx-auto opacity-75 hover:opacity-100 transition duration-150">
            </div>
    ...
    {{-- Script para Navbar Transparente/Sólido (Propio de Welcome) --}}
     <script>
        const navbar = document.getElementById('navbar');
        // Seleccionamos los elementos específicos dentro del navbar de welcome
        const logoImg = navbar ? navbar.querySelector('.logo-img') : null;
        const loginButton = navbar ? navbar.querySelector('.login-button') : null; // El botón de acceso admin

        const handleScroll = () => {
            if (!navbar) return;
            if (window.scrollY > 50) { // Cuando se hace scroll
                navbar.classList.add('bg-gray-900', 'shadow-md');
                navbar.classList.remove('bg-transparent');
                // Podrías cambiar el botón de login a colores oscuros si hiciera falta
                // if (loginButton) loginButton.classList.add('...');
            } else { // Cuando está arriba
                navbar.classList.remove('bg-gray-900', 'shadow-md');
                navbar.classList.add('bg-transparent');
                // Volver el botón de login a su estado original si lo cambiaste
                // if (loginButton) loginButton.classList.remove('...');
            }
        };
    ...
    {{-- @stack('scripts') --}} {{-- No necesitamos stack si no usamos layout --}}
</body>
</html>