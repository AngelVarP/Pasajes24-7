...
<head>
    ...
    {{-- Título adaptado a la ruta --}}
    <title>Editar Ruta: {{ $ruta->origen->nombre }} &rarr; {{ $ruta->destino->nombre }} - Admin</title>

    {{-- Estilos (copiados de tu template de admin) --}}
    ...
    <div class="flex min-h-screen">
        ...
        <aside class="admin-sidebar w-64 text-gray-300 hidden md:flex md:flex-col fixed inset-y-0 z-50 shadow-lg">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                {{-- Enlace de "Gestionar Rutas" (Activo) --}}
        ...
        <div class="flex-1 flex flex-col md:ml-64">
            ...
            <header class="bg-white shadow-sm h-16 flex items-center justify-between px-6 border-b border-gray-200">
                 <div class="flex items-center">
                     ...
                     <h1 class="header-title text-2xl font-extrabold tracking-tight">Editar Ruta</h1>
                 </div>
            ...
            <main class="flex-grow p-6 lg:p-10 main-content-bg">
                ...
                <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-4xl mx-auto">
                    ...
                    <h2 class="text-2xl font-semibold text-gray-800 mb-6 border-b pb-4 border-gray-200 flex items-center">
                         ...
                         Detalles de la Ruta: {{ $ruta->origen->nombre }} &rarr; {{ $ruta->destino->nombre }}
                    </h2>
                    ...
                    <form action="{{ route('admin.rutas.update', $ruta) }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="origen_id" id="origen_id" required
                                    ...
                                    @foreach ($ciudades as $ciudad)
                                        {{-- Usa old() o el valor de la ruta --}}
                                        <option value="{{ $ciudad->id }}" {{ (old('origen_id') ?? $ruta->origen_id) == $ciudad->id ? 'selected' : '' }}>
                                            {{ $ciudad->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="destino_id" id="destino_id" required
                                    ...
                                    @foreach ($ciudades as $ciudad)
                                        {{-- Usa old() o el valor de la ruta --}}
                                        <option value="{{ $ciudad->id }}" {{ (old('destino_id') ?? $ruta->destino_id) == $ciudad->id ? 'selected' : '' }}>
                                            {{ $ciudad->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('destino_id') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                            </div>
                            ...
                            @error('origen_id')
                                ...
                                @endif
                                @if ($message === 'Ya existe otra ruta con este origen y destino.')
                                     <p class="text-red-500 text-xs md:col-span-2">{{ $message }}</p>
                                @endif
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="duracion_estimada_minutos" id="duracion_estimada_minutos" required min="1" step="1"
                                   {{-- Usa old() o el valor de la ruta --}}
                                   value="{{ old('duracion_estimada_minutos', $ruta->duracion_estimada_minutos) }}" placeholder="Ej: 480"
                            ...
                            @error('duracion_estimada_minutos') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
